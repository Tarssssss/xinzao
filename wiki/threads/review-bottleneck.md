...
- **Swyx（Cognition）**：光谱的尽头。零人工写代码已经正常，下一个激进前沿是零人工 review——不看代码直接合入，他认为这是唯一可规模化的方式，前提是把开发流程翻过来补足测试和自动验证（2026-04-24-swyx-coding-agents-containment）。6 月再下一层：连 Git 都质疑该不该留——合并冲突占写码开销 20-40%，设想未来代码库像 Notion/Linear 数据库而非 .git，推测式发问（2026-06-13-swyx-future-codebase-git）。
