...
### 2026-06-13 routine 发现：feed 重新冒出旧博客（留给人定）
...
### 2026-07-15 routine 发现：7-13、7-14 两天无 issue，但两天的 routine 都确实跑过（更正，无需人核实是否漏跑）
...
### 2026-07-25 人工回填 7-20 至 7-23（并发的另一半，补记）

| # | 事实 | 说明 |
|---|------|------|
| — | 记录事实 | #39 提到的「另一个并发会话」即本次人工要求的 7-20 至 7-23 回填。回填侧同样只 `git add` 自己的 4 个 issue 文件 + `data/raw/days/2026-07-20..23` 快照，并等 7-25 routine 先 commit+push（b417b74）后再在其上提交，两侧文件互不重叠、未冲突 | 两个会话都手动避开了整目录 add，这次没踩到；但这**印证 #39 的「待人定」**——日更与回填并发确实会同时改到 `wiki/`、`src/data/issues/`，建议加锁或让回填走独立 git worktree |
| — | 记录事实 | 素材经 `backfill-snapshots.mjs --since` 从上游 git history 取回（7-20..23 每天 29-36 推 + 1 播客）；4 期共 11 story + 19 速览 | 其中 7-20 的 Stripe 播客、7-21 的 Booking 播客经去重发现分别已在 2026-07-11 / 07-12 收录（同一 episode），按主链接跳过、未重复成稿 |
